...
if(langSelect){ langSelect.value=currentLang; langSelect.addEventListener('change',()=>{ currentLang=langSelect.value; localStorage.setItem('lang',currentLang); loadI18n(currentLang); }); }
...
async function loadI18n(lang){
  try{
    ...
    const metaDesc=document.querySelector('meta[name="description"]'); if(metaDesc) metaDesc.setAttribute('content',dict.meta.description);
...
/* Projects filter: hide completely to avoid gaps */
...
function i18n(path){ const probe=document.querySelector(`[data-i18n="${path}"]`); return probe?.textContent||''; }
